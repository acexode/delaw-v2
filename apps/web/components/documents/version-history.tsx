...
export function VersionHistory({
  ...
  onClose: () => void;
  onSaved?: () => void;
  onRestore?: (version: DocumentVersionDetail) => void;
}) {
  const [versions, setVersions] = useState<DocumentVersionSummary[]>([]);
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [active, setActive] = useState<DocumentVersionDetail | null>(null);
  ...
  const view = async (id: string) => {
    ...
    setActive(null);
    try {
      const res = await documentsApi.getVersion(documentId, id);
      setActive(res.version);
    } catch {
      setActive(null);
    } finally {
      setViewing(false);
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <div className="font-serif text-[16px] font-semibold text-text-cream">
                {active ? `Version v${active.version}` : "Version history"}
              </div>
              <div className="text-[12px] text-text-muted">
                {active
                  ? `${relativeTime(active.createdAt)} · ${active.authorName ?? "Unknown"}`
          ...
          <>
            <div className="flex-1 overflow-y-auto px-5 py-4">
              {viewing ? (
                ...
                <div
                  className="delaw-editor text-[13px] text-text-body"
                  dangerouslySetInnerHTML={{
                    __html:
                      active.contentHtml ||
                      "<p class='text-text-muted'>This version has no content.</p>",
                  }}
                />
              ) : null}
            </div>
            {active && onRestore && (
              <div className="flex-none border-t border-line-subtle px-5 py-3">
                <button
                  type="button"
                  onClick={() => onRestore(active)}
                  className="flex h-9 w-full items-center justify-center gap-1.5 rounded-lg border border-gold/40 bg-gold/10 text-[12.5px] font-semibold text-gold hover:bg-gold/15"
                >
                  <History size={14} />
                ...
                <div className="flex flex-col gap-2">
                  {versions.map((v) => (
                    <button
                      key={v.id}
                      type="button"
                      onClick={() => void view(v.id)}
                      className="rounded-lg border border-line bg-bg-750 p-3 text-left transition-colors hover:border-line-accent hover:bg-bg-hover"
                    >
